...
namespace EasyTGL {
	...
	*/
	...
	const std::string to_string[5000] = {
	...
	vector<char> MAIN_BUFFER(EASYTGL_DEFAULT_MAIN_BUFFER_SIZE);	///< 程序运行期间, 所有stdout输出的缓冲区
...
#pragma endregion
	...
	private:
		...
		std::vector<rgb> frame_buffer;	///< 当前帧的RGB缓冲区
		std::vector<rgb> lastframe_frame_buffer;	///< 上一帧的RGB缓冲区, 用于差量绘制优化
	...
	*/
	void clear_all() { printf("\033[2J"); fflush(stdout); }
}